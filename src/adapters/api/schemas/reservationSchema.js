...
export const changeReservationPrice = {
  totalAmount: {
    in: ["body"],
    exists: {
      bail: true,
      errorMessage: "Total amount must be provided",
    },
    isArray: {
      bail: true,
      errorMessage: "Total amount must be an array",
    },
    custom: {
      options: values => {
        if (!Array.isArray(values)) throw new Error("Must be an array");

        // Shape of object == {id_1: amount_1, id_2: amount_2}
        values.forEach(value => {
          const validKeys = ["room_type_id", "amount"];
          const keysValid = Object.keys(value).every(key =>
            validKeys.includes(key)
          );

          const valuesValid = Object.values(value).every(
            value => typeof parseFloat(value) === "number" && !isNaN(value)
          );

          if (!keysValid) throw new Error("Object keys must be integers");
          if (!valuesValid) throw new Error("Object values must be numbers");
        });

        return true;
      },
      errorMessage: "Invalid object key-value pairs.",
    },
  },
};
